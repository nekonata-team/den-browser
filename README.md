...
- Hold web material in a Profile-owned Den-level Drawer, see its item count, filter Items by title or URL with `/` in Den Mode, expand one live preview at a time, and later place it as a Board or discard it. Den Mode remains toggleable inside the Drawer: it moves keyboard ownership between Drawer Items and the expanded Preview without ending the Preview's live state. Links opened from other apps enter the Drawer without changing the current Desk layout. An expanded preview stays live when the Drawer closes during the current app run, and returns with a new live runtime when first opened after relaunch.
