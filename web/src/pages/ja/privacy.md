...
## 3. 任意のコンテンツブロック機能（uBlock Origin Lite）について

Den Browserは、**uBlock Origin Lite（uBOL）** を必要に応じてインストールして利用できます。
* **任意インストール・オプトイン**: uBOLはオンデマンドでダウンロードされ、コンテンツブロックは初期状態では無効です。設定画面でインストールと有効化を明示的に行う必要があります。
...
* **2026年9月22日**: uBlock Origin Liteがアプリに同梱されず、オンデマンドでインストールされることを明記。
