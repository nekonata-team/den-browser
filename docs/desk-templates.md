...
`Manage Templates…` switches the New Desk panel to an inline management view. It supports search, deletion, pointer drag reordering, and keyboard Move Up / Move Down actions. Reordering is disabled while search is active. Built-in Desk Templates are visible during selection but are not managed.
