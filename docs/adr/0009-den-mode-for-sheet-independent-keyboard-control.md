...
- `n` or Space opens a new board, Shift plus `n` opens a new desk, and `b` opens Save Desk as Template when the Focused Desk has Boards. `Command` + `T` also opens the Open Board panel from every keyboard context, including Den Mode and Overview. Their panels suspend Den Mode. Creating the Board or Desk returns to Sheet Input; saving a Desk Template or canceling a panel returns to Den Mode.
