...
- `z` toggles Zen View without leaving Den Mode. Zen View is window-local runtime state, hides the Desk switcher and Profile control, and gives their upper display area to Boards. It does not hide the titlebar, Den Mode or Held Board signals, Board controls, panels, Overview, or the Keyboard Shortcuts guide. Key repeat does not repeatedly toggle Zen View.
