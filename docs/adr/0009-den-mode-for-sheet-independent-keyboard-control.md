...
- `n` or Space opens a new board and Shift plus `n` opens a new desk. `Command` + `T` also opens the Open Board panel from every keyboard context, including Den Mode and Overview. Their panels suspend Den Mode. Creating the Board or Desk returns to Sheet Input; canceling returns to Den Mode.
