...
Because Den Browser explicitly avoids the surface area of a general-purpose browser—such as opening local files as Sheets or acting as a default document reader—supporting the `file://` scheme falls outside of its scope. Doing so would introduce substantial technical and security complexity under the macOS App Sandbox:
...
Downloading a file from an HTTP or HTTPS Current Sheet is distinct from navigating a Sheet to a local file. Per [ADR 0027](./0027-support-deliberate-downloads-without-managing-them.md), a download uses a user-selected destination and does not persist a `file://` URL in `DenState`, a Desk Preset, or a Sheet Stack.
