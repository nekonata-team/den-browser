...
`Command`-clicking an HTTP or HTTPS link in a Current Sheet creates an adjacent Board without changing the Focused Board. Adding Shift focuses the new Board. This preserves the established macOS browser distinction between opening a background browsing context and opening one for immediate use while expressing both outcomes as Board placement. Option-click is reserved for Den's distinct Drawer capture operation rather than Board creation.
...
`Command` + `1` through `9`, and `Command` + `0`, focus Desk 1 through 10 from Sheet Input. This reuses the established browser muscle memory for selecting a numbered browsing context while applying it to Den's numbered Desks. Desk 10 uses `0` to preserve the familiar ten-position sequence.
