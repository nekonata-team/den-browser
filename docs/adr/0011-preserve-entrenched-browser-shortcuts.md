...
`Command`-clicking an HTTP or HTTPS link in a Current Sheet creates an adjacent Board without changing the Focused Board. Adding Shift focuses the new Board. This preserves the established macOS browser distinction between opening a background browsing context and opening one for immediate use while expressing both outcomes as Board placement. Option-click is not claimed for Board creation.
