...
Duplicating a zmx Board creates an independent zmx session instead of reconnecting to the source session. Den keeps the source Working Directory and derives the new name from the root session: entering `vi` for `den` creates `den-vi`, while duplicating that child and entering `nvim` creates the sibling `den-nvim`. Suffixes accept letters, numbers, `-`, `_`, and `.`. The root Board has no root label; child Boards persist the root session name and set the runtime zmx label `den.root` when their session starts. An empty suffix uses a numeric name, and any name collision is advanced to the next available number. Ordinary Terminal, Web, and Zellij duplication behavior is unchanged.
