...
Creating a desk from the New Desk panel inserts it after the focused desk and then focuses the new desk. `Control` + `Option` + `N` opens this panel globally, including while overview is visible; overview itself has no dedicated creation mode. A held board remains held across desk creation so a user can hold a board, create its destination desk, and then place it.
