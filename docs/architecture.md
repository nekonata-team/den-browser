...
`IPC` owns socket listening (`DenSocketServer`), request handling (`DenIPCService`), and ambient target resolution (`DenIPCTargetResolver`) connecting the bundled `den` CLI and external shells to active Profile workspaces, following [ADR 0047](./adr/0047-integrate-den-cli.md). It operates on domain operations without leaking WebKit or UI presentation internals. `Notification` owns transient Terminal Board event notifications and user alerts in the Den notification list.
