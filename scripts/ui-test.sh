...
run_test() {
    rtk test "xcodebuild test -project '$project' -scheme '$scheme' -destination 'platform=macOS' -derivedDataPath '$derived_data' -enableCodeCoverage NO -only-testing:'$1'"
}
...
rtk test "xcodebuild test -project '$project' -scheme '$scheme' -destination 'platform=macOS' -derivedDataPath '$derived_data' -enumerate-tests -test-enumeration-format json -test-enumeration-output-path '$enumeration'" >/dev/null
